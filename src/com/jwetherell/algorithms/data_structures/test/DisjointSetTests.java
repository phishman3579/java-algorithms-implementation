...
public class DisjointSetTests {
    ...
    public void testDisjointSet1() {
        ...
        final DisjointSet<Integer> set = new DisjointSet<Integer>();
        final DisjointSet.Item<Integer>[] items = new DisjointSet.Item[array.length];
        for (int i=0; i<items.length; i++) {
            final int v = array[i];
            final DisjointSet.Item<Integer> s = set.makeSet(v);
            items[i] = s;
            final DisjointSet.Item<Integer> f = set.find(s);
        ...
        for (int i=0; i<half; i++) {
            final DisjointSet.Item<Integer> item = items[i];
            final DisjointSet.Item<Integer> result = set.find(item);
        ...
        for (int i=0; i<items.length; i++) {
            final DisjointSet.Item<Integer> item = items[i];
            superSet = set.union(superSet, item);
        }
        System.out.println(toString(items));
        for (int i=0; i<items.length; i++) {
            final DisjointSet.Item<Integer> item = items[i];
